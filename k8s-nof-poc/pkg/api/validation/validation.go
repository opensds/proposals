...
func validateVolumeSource(source *api.VolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if source.NVMEOF != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, field.Forbidden(fldPath.Child("nvmeof"), "may not specify more than 1 volume type"))
		} else {
			numVolumes++
			allErrs = append(allErrs, validateNVMEOFVolumeSource(source.NVMEOF, fldPath.Child("nvmeof"))...)
		}
	}
...
func validateNVMEOFVolumeSource(nvmeof *api.NVMEOFVolumeSource, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	if len(nvmeof.Transport) == 0 {
		allErrs = append(allErrs, field.Required(fldPath.Child("transport"), ""))
	}
	if len(nvmeof.TrAddr) == 0 {
		allErrs = append(allErrs, field.Required(fldPath.Child("traddr"), ""))
	}
	if nvmeof.TrSvcId < 0 {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("trsvcid"), nvmeof.TrSvcId, "Negative value not allowed"))
	}
	if len(nvmeof.Nqn) == 0 {
		allErrs = append(allErrs, field.Required(fldPath.Child("nqn"), ""))
	}
	return allErrs
}
...
func ValidatePersistentVolume(pv *api.PersistentVolume) field.ErrorList {
	...
	if pv.Spec.NVMEOF != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, field.Forbidden(specPath.Child("nvmeof"), "may not specify more than 1 volume type"))
		} else {
			numVolumes++
			allErrs = append(allErrs, validateNVMEOFVolumeSource(pv.Spec.NVMEOF, specPath.Child("nvmeof"))...)
		}
	}
