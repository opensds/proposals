...
func describeVolumes(volumes []api.Volume, w *PrefixWriter, space string) {
	...
	for _, volume := range volumes {
		...
		case volume.VolumeSource.NVMEOF != nil:
			printNVMEOFVolumeSource(volume.VolumeSource.NVMEOF, w)
...
func printNVMEOFVolumeSource(nvmeof *api.NVMEOFVolumeSource, w *PrefixWriter) {
	w.Write(LEVEL_2, "Type:\tNVMEOF (an NVMEOF Disk resource that is attached to a kubelet's host machine and then exposed to the pod)\n"+
		"    Transport:\t%v\n"+
		"    TrAddr:\t%v\n"+
		"    TrSvcId:\t%v\n"+
		"    Nqn\t%v\n"+
		"    FSType:\t%v\n"+
		"    ReadOnly:\t%v\n",
		nvmeof.Transport, nvmeof.TrAddr, nvmeof.TrSvcId, nvmeof.Nqn, nvmeof.FSType, nvmeof.ReadOnly)
}
...
func (d *PersistentVolumeDescriber) Describe(namespace, name string, describerSettings DescriberSettings) (string, error) {
	...
	return tabbedString(func(out io.Writer) error {
		...
		case pv.Spec.NVMEOF != nil:
			printNVMEOFVolumeSource(pv.Spec.NVMEOF, w)
