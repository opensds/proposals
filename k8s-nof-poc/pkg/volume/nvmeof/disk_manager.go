...
type diskManager interface {
	MakeGlobalPDName(disk nvmeofDisk) string
	// Attaches the disk to the kubelet's host machine.
	AttachDisk(b nvmeofDiskMounter) error
	// Detaches the disk from the kubelet's host machine.
	DetachDisk(disk nvmeofDiskUnmounter, mntPath string) error
}

// utility to mount a disk based filesystem
func diskSetUp(manager diskManager, b nvmeofDiskMounter, volPath string, mounter mount.Interface, fsGroup *int64) error {
	globalPDPath := manager.MakeGlobalPDName(*b.nvmeofDisk)
...
func diskTearDown(manager diskManager, c nvmeofDiskUnmounter, volPath string, mounter mount.Interface) error {
	notMnt, err := mounter.IsLikelyNotMountPoint(volPath)
	if err != nil {
		glog.Errorf("cannot validate mountpoint %s", volPath)
		return err
