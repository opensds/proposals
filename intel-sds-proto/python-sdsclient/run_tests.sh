...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run sdsclient's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./sdsclient/tests"
    fi
  ...
  if [ setup.cfg -nt sdsclient.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='sdsclient/*' --omit='sdsclient/openstack/common/*' -d covhtml -i
  fi
