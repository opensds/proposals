...
class ShellTest(utils.TestCase):
    ...
    def shell(self, argstr):
        ...
        try:
            ...
            _shell = shell.OpenStackSdsShell()
    ...
    def test_help(self):
        required = [
            '.*?^usage: ',
            '.*?(?m)^\s+create\s+Creates a sds.',
            '.*?(?m)^Run "sds help SUBCOMMAND" for help on a subcommand.',
        ]
...
class SdsClientArgumentParserTest(utils.TestCase):

    def test_ambiguity_solved_for_one_visible_argument(self):
        parser = shell.SdsClientArgumentParser(add_help=False)
