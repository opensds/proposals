...
CINDER_ENDPOINT = 'http://www.sds.com/v1'
...
def _get_normalized_token_data(**kwargs):
    ...
    ref['sds_url'] = ref.get('sds_url', CINDER_ENDPOINT)
...
def generate_v2_project_scoped_token(**kwargs):
    ...
    o['access']['serviceCatalog'] = [
        {'endpoints': [
            {'publicURL': 'public_' + ref.get('sds_url'),
             'internalURL': 'internal_' + ref.get('sds_url'),
        ...
        {'endpoints': [
            {
                ...
                'interface': 'public',
                'region': 'RegionOne',
                'url': 'public_' + ref.get('sds_url')
            },
            {
                'id': uuid.uuid4().hex,
                'interface': 'internal',
                'region': 'RegionOne',
                'url': 'internal_' + ref.get('sds_url')
            },
            {
                'id': uuid.uuid4().hex,
                'interface': 'admin',
                'region': 'RegionOne',
                'url': 'admin_' + ref.get('sds_url')
            }],
         'id': uuid.uuid4().hex,
         'type': 'network'},
        {'endpoints': [
