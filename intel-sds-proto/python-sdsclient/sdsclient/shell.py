...
from sdsclient import exceptions as exc
from sdsclient import utils
import sdsclient.auth_plugin
import sdsclient.extension
from sdsclient.openstack.common import strutils
from sdsclient.openstack.common.gettextutils import _
from sdsclient.v1 import shell as shell_v1
...
DEFAULT_OS_SDS_API_VERSION = "1"
DEFAULT_SDS_ENDPOINT_TYPE = 'publicURL'

DEFAULT_SDS_SERVICE_TYPE = 'storage_controller_v1'

logging.basicConfig()
logger = logging.getLogger(__name__)


class SdsClientArgumentParser(argparse.ArgumentParser):

    def __init__(self, *args, **kwargs):
        super(SdsClientArgumentParser, self).__init__(*args, **kwargs)
    ...
    def _get_option_tuples(self, option_string):
        ...
        result = super(SdsClientArgumentParser, self)._get_option_tuples(
            option_string)
...
class OpenStackSdsShell(object):

    def get_base_parser(self):
        parser = SdsClientArgumentParser(
            prog='sds',
            description=__doc__.strip(),
            epilog='Run "sds help SUBCOMMAND" for help on a subcommand.',
            add_help=False,
        ...
        parser.add_argument('--version',
                            action='version',
                            version=sdsclient.__version__)

        parser.add_argument('--debug',
                            action='store_true',
                            default=utils.env('SDSCLIENT_DEBUG',
                                              default=False),
        ...
        parser.add_argument('--service-type',
                            ...
                            help='Service type. '
                            'For most actions, default is sds.')
        parser.add_argument('--service_type',
                            help=argparse.SUPPRESS)

        parser.add_argument('--service-name',
                            metavar='<service-name>',
                            default=utils.env('SDS_SERVICE_NAME'),
                            help='Service name. '
                            'Default=env[SDS_SERVICE_NAME].')
        parser.add_argument('--service_name',
                            help=argparse.SUPPRESS)

        parser.add_argument('--endpoint-type',
                            metavar='<endpoint-type>',
                            default=utils.env('SDS_ENDPOINT_TYPE',
                            default=DEFAULT_SDS_ENDPOINT_TYPE),
                            help='Endpoint type, which is publicURL or '
                            'internalURL. '
                            'Default=nova env[SDS_ENDPOINT_TYPE] or '
                            + DEFAULT_SDS_ENDPOINT_TYPE + '.')

        parser.add_argument('--endpoint_type',
                            help=argparse.SUPPRESS)

        parser.add_argument('--os-sds-api-version',
                            metavar='<sds-api-ver>',
                            default=utils.env('OS_SDS_API_VERSION',
                                              default=None),
                            help='Block Storage API version. '
                            'Valid values are 1 or 2. '
                            'Default=env[OS_SDS_API_VERSION].')
        parser.add_argument('--os_sds_api_version',
                            help=argparse.SUPPRESS)
        ...
        sdsclient.auth_plugin.discover_auth_systems()
        sdsclient.auth_plugin.load_auth_system_opts(parser)
    ...
    def _append_global_identity_args(self, parser):
        ...
        parser.add_argument('--insecure',
                            default=utils.env('SDSCLIENT_INSECURE',
                                              default=False),
                            action='store_true',
                            help=argparse.SUPPRESS)
    ...
    def _discover_extensions(self, version):
        ...
        for name, module in itertools.chain(
            ...
            extension = sdsclient.extension.Extension(name, module)
    ...
    def _discover_via_python_path(self, version):
        for (module_loader, name, ispkg) in pkgutil.iter_modules():
            if name.endswith('python_sdsclient_ext'):
    ...
    def main(self, argv):
        ...
        if not options.os_sds_api_version:
            # Environment variable OS_SDS_API_VERSION was
            # not set and '--os-sds-api-version' option doesn't
            # specify a value.  Fall back to default.
            options.os_sds_api_version = DEFAULT_OS_SDS_API_VERSION
            api_version_input = False

        # build available subcommands based on version
        self.extensions = self._discover_extensions(
            options.os_sds_api_version)
        ...
        if os_auth_system and os_auth_system != "keystone":
            auth_plugin = sdsclient.auth_plugin.load_plugin(os_auth_system)
        else:
            auth_plugin = None

        if not endpoint_type:
            endpoint_type = DEFAULT_SDS_ENDPOINT_TYPE

        if not service_type:
            service_type = DEFAULT_SDS_SERVICE_TYPE
        ...
        self.cs = client.Client(options.os_sds_api_version, os_username,
        ...
        # --os-sds-api-version or with the OS_SDS_API_VERSION environment
        # variable.  Fail safe is to use the default API setting.
        try:
            endpoint_api_version = \
                self.cs.get_sds_api_version_from_endpoint()
            if endpoint_api_version != options.os_sds_api_version:
                msg = (("OpenStack Block Storage API version is set to %s "
                        "but you are accessing a %s endpoint. "
                        "Change its value through --os-sds-api-version "
                        "or env[OS_SDS_API_VERSION].")
                       % (options.os_sds_api_version, endpoint_api_version))
                raise exc.InvalidAPIVersion(msg)
        except exc.UnsupportedVersion:
            endpoint_api_version = options.os_sds_api_version
    ...
    def do_bash_completion(self, args):
        ...
        sds.bash_completion script does not have to hard code them.
        """
        commands = set()
        options = set()
...
def main():
    try:
        if sys.version_info >= (3, 0):
            OpenStackSdsShell().main(sys.argv[1:])
        else:
            OpenStackSdsShell().main(map(strutils.safe_decode,
                                            sys.argv[1:]))
    except KeyboardInterrupt:
        print("... terminating sds client", file=sys.stderr)
        sys.exit(130)
