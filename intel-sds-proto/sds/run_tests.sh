...
function run_tests {
  ...
  if [ setup.cfg -nt sds.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='sds/*' --omit='sds/openstack/common/*' -d covhtml -i
  fi
