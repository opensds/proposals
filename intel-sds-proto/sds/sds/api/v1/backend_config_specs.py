...
class StorageBackendConfigSpecsTemplate(xmlutil.TemplateBuilder):
    def construct(self):
        root = xmlutil.make_flat_dict('config_specs', selector='config_specs')
        return xmlutil.MasterTemplate(root, 1)


class StorageBackendConfigSpecTemplate(xmlutil.TemplateBuilder):
    def construct(self):
...
class StorageBackendConfigSpecsController(wsgi.Controller):
    """The storage backend config specs API controller for the OpenStack API."""

    def _get_config_specs(self, context, backend_id):
        config_specs = storage_backends.get_backend_config_specs(context, backend_id)
        specs_dict = {}
        for key, value in config_specs.iteritems():
            specs_dict[key] = value
        return dict(config_specs=specs_dict)
    ...
    @wsgi.serializers(xml=StorageBackendConfigSpecsTemplate)
    def index(self, req, backend_id):
        """Returns the list of extra specs for a given volume type."""
        context = req.environ['sds.context']
        self._check_backend(context, backend_id)
        return self._get_config_specs(context, backend_id)

    @wsgi.serializers(xml=StorageBackendConfigSpecsTemplate)
    def create(self, req, backend_id, body=None):
        context = req.environ['sds.context']

        if not self.is_valid_body(body, 'config_specs'):
            raise webob.exc.HTTPBadRequest()

        self._check_backend(context, backend_id)
        specs = body['config_specs']
        self._check_key_names(specs.keys())
        storage_backends.create_backend_config_specs(context, backend_id, specs)
        notifier_info = dict(backend_id=backend_id, specs=specs)
        notifier = rpc.get_notifier('storageBackendExtraSpecs')
        notifier.info(context, 'storage_backend_config_specs.create',
                      notifier_info)
        return body
    ...
    def delete(self, req, backend_id, id):
        ...
        try:
            storage_backends.destroy_backend_config_specs(context, backend_id, id)
        ...
        notifier.info(context,
                      'storage_backend_config_specs.delete',
                      notifier_info)
...
def create_resource():
    return wsgi.Resource(StorageBackendConfigSpecsController())
