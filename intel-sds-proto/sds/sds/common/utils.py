...
from sds.openstack.common import lockutils
from sds.openstack.common import log as logging
from sds.openstack.common import processutils
...
synchronized = lockutils.synchronized_with_prefix('sds-')
...
def find_config(config_path):
    ...
    :raises: `sds.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(CONF.state_path, "etc", "sds", config_path),
        os.path.join(CONF.state_path, "etc", config_path),
        os.path.join(CONF.state_path, config_path),
        "/etc/sds/%s" % config_path,
    ]
...
def sdsdir():
    import sds
    return os.path.abspath(sds.__file__).split('sds/__init__.py')[0]
...
def monkey_patch():
    ...
    Example: 'sds.api.ec2.cloud:' \
     sds.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See sds.openstack.common.notifier.api.notify_decorator)
...
def get_root_helper():
    return 'sudo sds-rootwrap %s' % CONF.rootwrap_config
